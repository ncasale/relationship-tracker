...
export class AuthService {
    ...
    saveUser(user: User) {
        ...
        return this.http.post('http://52.91.114.12:3000/auth', body, {headers: headers})
    ...
    loginUser(user: User) {
        ...
        return this.http.post('http://52.91.114.12:3000/auth/login', body, {headers:headers})
            .map((response: Response) => response.json())
            .catch((error: Response) => {
                this.errorService.handleError(error.json());
                return Observable.throw(error.json())
    ...
    loginWithToken() {
        ...
        //Create request
        return this.http.post('http://52.91.114.12:3000/auth/loginwithtoken' + token, body, {headers:headers})
    ...
    getUser(userId: string) {
        ...
        const token = this.getToken();
        //Create request
        return this.http.post('http://52.91.114.12:3000/auth/getuser/' + userId + token, body, {headers:headers})
            .map((response: Response) => {
    ...
    getUserInvites() {
        ...
        const token = this.getToken();
        //Create request
        return this.http.post('http://52.91.114.12:3000/auth/getuserinvites/' + token, body, {headers:headers})
            .map((response: Response) => {
    ...
    changePassword(oldPassword: string, newPassword: string) {
        ...
        const token = this.getToken();
        //Create request
        return this.http.patch('http://52.91.114.12:3000/auth/changepassword' + token, body, {headers:headers})
            .map((response: Response) => {
                return response.json().obj;
            })
            .catch((error: Response) => {
